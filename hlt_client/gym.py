...
def connect(db_path=None):
    if not db_path:
        db_path = os.path.join(appdirs.user_data_dir(
            APP_NAME, APP_AUTHOR), 'gym.db')
...
def rerank_bots(conn):
    ...
    for index, bot in enumerate(all_bots):
        rank = index + 1
        conn.execute('update bots set rank = ? where id = ?',
                     (rank, bot['id']))
...
def register_bot(conn, name, path):
    ...
    existing = conn.execute(
        'select * from bots where name = ?', (name,)).fetchall()
    if existing:
    ...
    for i in range(iterations):
        with connect(db_path) as conn:
            ...
            if len(all_bots) < MIN_PLAYERS:
                output.error(
                    'Need at least {} bots registered to play.'.format(MIN_PLAYERS))
                sys.exit(1)
        ...
        output.output('Played {}/{} matches...'.format(i + 1, iterations),
                      ...
                      results=results,
                      participants=bots)
    output.output('Done playing games.',
                  progress=iterations, iterations=iterations)
...
def get_rank_history(conn, bot_id):
    records = conn.execute(
        'select datetime, rank, mu, sigma from rank_history where bot_id = ?', (bot_id,))
...
def main(args):
    if args.gym_mode == BOTS_MODE:
        ...
        def _prettyprint_bot(bot):
            return "\n".join([
                'Bot "{}" (ID {}, version {})'.format(
                    bot['name'], bot['id'], bot['version']),
                'Rank {} (score={:.02f}, μ={:.02f}, σ={:.02f})'.format(
                    bot['rank'], bot['mu'] - 3*bot['sigma'], bot['mu'], bot['sigma']),
        ...
        with connect(args.db_path) as conn:
            bots = list_bots(conn)
            output.print_list("Registered Bots:", bots,
                              formatter=_prettyprint_bot)
        ...
        def _prettyprint_match(match):
            winner = [p for p in match['participants']
                      if p['id'] == match['winner']][0]
            return 'Match #{}: "{}" beat {}\nMap Size: {}x{}\nReplay: {}'.format(
                match['id'],
                winner['name'],
        ...
        with connect(args.db_path) as conn:
            ...
            output.print_list("Games Played:", matches,
                              formatter=_prettyprint_match)
...
def parse_arguments(subparser):
    ...
    register_parser = gym_subparser.add_parser(
        REGISTER_MODE, help='Register or update a bot with the gym.')
    ...
    deregister_parser = gym_subparser.add_parser(
        DEREGISTER_MODE, help='Delete a bot.')
    deregister_parser.add_argument('name', type=str,
                                   help="The name of the bot to delete.")

    evaluate_parser = gym_subparser.add_parser(
        EVALUATE_MODE, help='Run games with bots in the gym.')
    ...
    stats_parser = gym_subparser.add_parser(
        STATS_MODE, help='Get stats from the gym.')
    stats_parser.add_argument('query', nargs='?', type=str,
                              help="An SQL query to run (this is NOT SANITIZED in any way!)")

    bots_parser = gym_subparser.add_parser(
        BOTS_MODE, help='List registered bots.')
